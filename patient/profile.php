...
if (isset($_POST['submit'])) {
    ...
    $uploadOk = 1;
    $imageData = file_get_contents($_FILES["profileImage"]["tmp_name"]); // Read the image data
    ...
    $imageType = $_FILES["profileImage"]["type"]; // Get the image MIME type
    ...
    // if everything is ok, try to upload file
        ...
        $stmt = $con->prepare("UPDATE patient SET patientFirstName=?, patientLastName=?, patientMaritialStatus=?, patientDOB=?, patientGender=?, patientAddress=?, patientPhone=?, patientEmail=?, profileImage=? WHERE icPatient=?");
        $stmt->bind_param("ssssssisss", $patientFirstName, $patientLastName, $patientMaritialStatus, $patientDOB, $patientGender, $patientAddress, $patientPhone, $patientEmail, $imageData, $_SESSION['patientSession']);
        $stmt->execute();
        $stmt->close();
					...
					<div class="row">
						<div class="col-md-3 col-sm-3">
							...
							<div class="user-wrapper">
								<!-- Display the uploaded image from the database -->
<?php
// Fetch the image data from the database
$result = mysqli_query($con, "SELECT profileImage FROM patient WHERE icPatient = ".$_SESSION['patientSession']);
if ($result && mysqli_num_rows($result) > 0) {
    $row = mysqli_fetch_assoc($result);
    $profileImage = $row['profileImage'];
    // Output the image data as the source of the image
    echo '<img src="data:image/jpeg;base64,'.base64_encode($profileImage).'" alt="Profile Image" class="img-responsive" />';
} else {
    // If no image found, display a placeholder or default image
    echo '<img src="path_to_default_image.jpg" alt="Default Image" class="img-responsive" />';
}
?>
								<div class="description">
									<h4><?php echo $userRow['patientFirstName']; ?> <?php echo $userRow['patientLastName']; ?></h4>
									<h5> Type:<strong> Patient </strong></h5>
									<p>
